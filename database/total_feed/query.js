...
export const getTotalFeed = connect(async (con, req) =>{
    ...
    let query = "SELECT post.id as postId, post.title, postAttachment.url, (SELECT COUNT(*) FROM `like` WHERE `like`.post_id = post.id ) as likeCount, " +
        "IF (`like`.user_id = ?, TRUE, FALSE) as selectLike, postVersion.version " +
        "FROM post " +
        "LEFT JOIN `like` ON post.id = `like`.post_id AND `like`.user_id = ? " +
        "LEFT JOIN postVersion ON post.id = postVersion.post_id " +
        "LEFT JOIN postAttachment ON postVersion.postAttachment_id = postAttachment.id " +
        "WHERE postAttachment.type = 0  AND postVersion.version = (SELECT MAX(version) FROM postVersion WHERE postVersion.post_id = post.id) " +
        "GROUP BY post.id, postAttachment.url, `like`.user_id,postVersion.version ; "; // 쿼리문 작성시 맨 뒤에 공백 넣어줄 것
